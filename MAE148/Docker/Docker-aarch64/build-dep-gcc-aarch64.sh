...
readonly TOOLCHAIN_FILE=aarch64-toolchain.cmake  # Update this with the correct path
...
configure () {
    local CMAKEFLAGS="
        ...
        -D CUDA_ARCH_PTX= \
        -D CUDA_FAST_MATH=ON \
        -D CUDNN_INCLUDE_DIR=/usr/include/${TARGET_ARCH}-linux-gnu \
...
# Cross-compilation specific packages
apt-get install -y --no-install-recommends \
